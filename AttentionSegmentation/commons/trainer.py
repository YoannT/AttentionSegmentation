...
class Trainer(object):
    def __init__(self,
        ...
        self.classifier1, self.classifier2 = create_classifiers()

        import pickle as pkl

        with open('/home/ytaille/AttentionSegmentation/vocab1.pkl', 'rb') as f:
            self.vocabularies1 = pkl.load(f)

        with open('/home/ytaille/AttentionSegmentation/vocab2.pkl', 'rb') as f:
            self.vocabularies2 = pkl.load(f)
    ...
    def _train_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        train_predictions = self._segmenter.get_predictions(
                    instances=shuffled_train_data, # may need to match train data index with iterator index here
        ...
        print("TRAIN TRAINER AYYYYYYYYY")
        ...
        for batch in train_generator_tqdm:
            ...
            tp = train_predictions[cpt_batch:cpt_batch+len(batch)]

            train_preds = [t["text"] for t in tp]
            train_texts = [' '.join(t) for t in train_preds]
            ...
            tp_text = [t['text'] for t  in tp]
            tp_labels = [t['gold_labels'] for t  in tp]

            save_to_ann(tp_text, tp_labels, '/home/ytaille/data/tmp/ws_inputs')
            ...
            docs, sentences, tokens, deltas, vocs = preprocess(
                ...
                bert_name=bert_name,
                vocabularies=self.vocabularies2,
            )
            ...
            with evaluating(self.classifier2):  # eval mode: no dropout, frozen batch norm, etc
                with torch.no_grad():  # no gradients -> faster
                    with tqdm(batcher.dataloader(batch_size=batch_size, device=device, sparse_sort_on="token_mask"), disable=not with_tqdm, mininterval=10.) as bar:
                        for p_batch in bar:
                            res = self.classifier2.forward(
                                tokens=p_batch["token"],
                                mask=p_batch["token_mask"],
                                groups=None,
                                return_scores=True,
                                return_embeds=False,
                            )
            ...
            best_probs = res['scores'].topk(dim=-1, k=1).values

            print(best_probs)

            print(best_probs.min(), best_probs.mean(), best_probs.max())
